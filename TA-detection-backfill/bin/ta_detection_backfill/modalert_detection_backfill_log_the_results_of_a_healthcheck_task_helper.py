...
def process_event(helper, *args, **kwargs):
    ...
    logger_file = LoggerFile(logger, "CAA-LTROAHT")
    helper.log_info("[CAA-LTROAHT-000] LOG level to: " + helper.log_level)
    helper.set_log_level(helper.log_level)
    ...
    for event in events:
        ...
        expected_fields = ["exec_time", "healthcheck_uid", "search_id", "app", "savedsearch_name", "scan_count", "event_count", "result_count"]
        ...
        event_message = {
            ...
            "type": "report",
            "created_time": now,
            "created_author": helper.settings["owner"],
            "job": {
                "type": "healthcheck_job",
                "sid": orig_sid,
                "timestamp": int(orig_exec_time),
                "timestamp_readable": datetime.datetime.fromtimestamp(orig_exec_time,tz=tzlocal.get_localzone()).strftime("%c %z"),
